    ...
    def get(self, request):
        return Response({
            ...
            "data": {
                ...
                "approved_projects": Project.objects.filter(status="PROPOSAL_APPROVED").count(),
                "pending_projects": Project.objects.filter(status="PENDING").count(),
                "supervisor_assigned_projects": Project.objects.filter(status="SUPERVISOR_ASSIGNED").count(),
                "revision_required_projects": Project.objects.filter(status="REVISION_REQUIRED").count(),
                "in_progress_projects": Project.objects.filter(status="IN_PROGRESS").count(),
                "ready_for_viva_projects": Project.objects.filter(status="READY_FOR_VIVA").count(),
